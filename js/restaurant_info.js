...
;(function() {
  ...
  form.addEventListener('submit', e => {
    ...
    restaurantHandler.reviewRestaurant(
      formData,
      reviewObject,
      (error, review) => {
        if (error) {
        ...
        const al = new alert('Thank you for your review', {
          showFor: 2000,
          classes: ['alert-box', 'alert-success'],
          id: 'message',
          bindTo: document.getElementById('restaurant-container')
        })
      }
    )
  })
  ...
  function createFavoriteButton() {
    ...
    favoriteButton.addEventListener('click', () => {
      currentRestaurant.is_favorite =
        currentRestaurant.is_favorite === 'true' ? 'false' : 'true'
      restaurantHandler.favoriteRestaurant(
        currentRestaurant,
        (error, result) => {
          if (error) {
            console.log(error)
  ...
  const displayError = (errorText = 'Something went wrong...', code) => {
    ...
    restaurantContainer.insertBefore(
      errorParagraph,
      restaurantContainer.firstChild
    )
  }
  ...
  const fillRestaurantHTML = (restaurant = currentRestaurant) => {
    ...
    sourceWebP.setAttribute(
      'srcset',
      restaurantHandler.imageUrlForRestaurant(restaurant) + '.webp'
    )
    ...
    // fill reviews
    restaurantHandler.getAllReviewsForRestaurant(
      restaurant.id,
      (error, reviews) => {
  ...
  const fillRestaurantHoursHTML = (
    operatingHours = currentRestaurant.operating_hours
  ) => {
  ...
  const createReviewHTML = review => {
    ...
    const dateTime = new Date(
      date == isNaN(review.updatedAt)
        ? review.updatedAt
        : parseInt(review.updatedAt)
    )
    date.innerHTML =
      dateTime.getMonth() +
      1 +
